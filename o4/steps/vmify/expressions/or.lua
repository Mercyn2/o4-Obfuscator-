...
return function(self, expr, funcDepth, n)
  ...
  if expr.rhs.isConstant then
    ...
    self:addStatement(self:setRegister(scope, resReg, A.OrExpression(self:register(scope, lhsReg), self:register(scope, rhsReg))), {resReg}, {lhsReg, rhsReg}, false)
  ...
  self:addStatement(self:setRegister(scope, self.POS_REGISTER,
    A.OrExpression(A.AndExpression(self:register(scope, lhsReg), A.NumberExpression(b2.id)), A.NumberExpression(b1.id))
